...
def scrape_cbc(headless=False):
    """Scrape CBC News and return data dictionary"""
    browser = BrowserUtility()

    try:
        print("Launching CBC News website...")
        page = browser.launch_browser(url="https://www.cbc.ca/news", headless=headless)

        cbc_page = CBCPage(page)
        cbc_page.wait_for_page_load()

        # Scroll page to load dynamic content
        cbc_page.scroll_page_after_load()

        print("\nGetting current page title and URL...")
        cbc_page.get_page_title()
        cbc_page.get_current_url()

        print("\n" + "="*80)
        print("Fetching recent news cards (within 6 hours or in minutes)...")
        print("="*80 + "\n")
        recent_cards = cbc_page.get_recent_news_cards()
        ...
        if recent_cards:
            ...
            for index, card in enumerate(recent_cards, 1):
                details = cbc_page.get_card_details(card)
        ...
        if news_cards:
            ...
            for card_key, card_details in filtered_news_cards.items():
                ...
                print(f"    'author': '{card_details.get('author', 'N/A')}'")
                print(f"    'time_updated': '{card_details.get('time_updated', 'N/A')}'")
                print(f"    'category': '{card_details.get('category', 'N/A')}'")
...
def test_cbc_navigation():
    """Test function for standalone execution"""
    data = scrape_cbc(headless=False)
